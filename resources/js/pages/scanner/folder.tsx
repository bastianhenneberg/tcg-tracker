...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Ordner-Scanner', href: '/scanner/folder' },
];
        ...
        <div className="flex items-center gap-2">
            ...
            >
                {ollamaStatus ? (
                    <Badge
                        variant={
                            ollamaStatus.available ? 'default' : 'destructive'
                        }
                        className="gap-1.5"
                        title={`${ollamaStatus.host} · ${ollamaStatus.model}`}
                    >
                        <span
                            className={`h-2 w-2 rounded-full ${ollamaStatus.available ? 'bg-success' : 'bg-destructive-subtle'}`}
                        />
                        {ollamaStatus.available
                            ? `KI-Server verbunden (${ollamaStatus.model})`
                            : 'KI-Server offline'}
                    </Badge>
                ) : null}
            </Deferred>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Liest gescannte Karten (PDF/Bilder) aus einem
                            Ordner, erkennt sie per KI und fügt Treffer ins
                            Inventar ein.
                        </p>
                    </div>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 sm:col-span-2">
                                <Switch
                                    ...
                                    onCheckedChange={(v) =>
                                        form.setData('dry_run', v)
                                    }
                                />
                                <Label htmlFor="dry_run">
                                    Nur testen (Dry-Run) – nichts ins Inventar
                                    schreiben, Dateien behalten
                                </Label>
                            </div>
                            ...
                            <div className="sm:col-span-2">
                                <Button
                                    type="submit"
                                    disabled={form.processing || isRunning}
                                >
                                    {(form.processing || isRunning) && (
                        ...
                        <CardContent className="space-y-3">
                            ...
                            {isRunning && (
                                <p className="text-sm text-muted-foreground">
                                    Verarbeitet: {scanStatus.pages ?? 0}{' '}
                                    Seite(n), {scanStatus.matched ?? 0} Treffer
                                    …
                                </p>
                            )}
                            ...
                            {scanStatus.summary && (
                                <>
                                    <div className="flex flex-wrap gap-4 text-sm">
                                        ...
                                        <span>
                                            Seiten:{' '}
                                            <b>{scanStatus.summary.pages}</b>
                                        </span>
                                        <span className="text-success">
                                            Importiert/Treffer:{' '}
                                            <b>{scanStatus.summary.matched}</b>
                                        </span>
                                        <span className="text-warning">
                                            Ohne Treffer:{' '}
                                            <b>
                                                {scanStatus.summary.unmatched}
                                            </b>
                                        </span>
                                        <span className="text-destructive">
                                            Fehlgeschlagen:{' '}
                                            <b>{scanStatus.summary.failed}</b>
                                        </span>
                                    </div>
