...
export default function DecksIndex({ game, decks, sort }: Props) {
    ...
    /* Beide Handler haengen nur an `game.slug`. Ohne `useCallback` sind sie
       bei jedem Rendern neu, und die Spalten-Memoisierung darunter laesst
       sich dann nicht erhalten — der React-Compiler ueberspringt die ganze
       Komponente. */
    const handleToggleInventory = useCallback(
        (e: React.MouseEvent, deck: DeckWithRelations) => {
            e.stopPropagation();
    ...
    const columns = useMemo<ColumnDef<DeckWithRelations, unknown>[]>(
                    ...
                    <div className="flex justify-end gap-1">
                        <Tooltip>
                            <TooltipTrigger asChild>
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    className={`h-8 w-8 ${row.original.is_inventory_active ? 'text-warning' : ''}`}
                                    onClick={(e) =>
                                        handleToggleInventory(e, row.original)
                                    }
                                >
                                    {row.original.is_inventory_active ? (
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                router.visit(
                                    `/g/${game.slug}/decks/${row.original.id}`,
                                );
                            }}
                        >
                        ...
                        <Button
                            ...
                            onClick={(e) => {
                                e.stopPropagation();
                                router.visit(
                                    `/g/${game.slug}/decks/${row.original.id}/builder`,
                                );
                            }}
                        >
        ...
        [game.slug, handleDelete, handleToggleInventory],
    );
