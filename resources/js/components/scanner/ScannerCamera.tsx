...
export function ScannerCamera({
    ...
    const bulkScanIntervalRef = useRef<ReturnType<typeof setInterval> | null>(
        null,
    );
    const bulkCountdownIntervalRef = useRef<ReturnType<
        typeof setInterval
    > | null>(null);
    ...
    const startCamera = async () => {
        try {
            ...
            setCameraSettings((prev) => ({
                ...prev,
                focusDistance:
                    currentSettings.focusDistance ?? prev.focusDistance,
    ...
    const applyCameraSetting = async (setting: Partial<CameraSettings>) => {
        ...
        try {
            ...
            if (setting.zoom !== undefined && cameraCapabilities.zoom)
                adv.zoom = setting.zoom;
            if (setting.focusMode !== undefined && cameraCapabilities.focusMode)
                adv.focusMode = setting.focusMode;
            if (
                setting.focusDistance !== undefined &&
                cameraCapabilities.focusDistance
            )
                adv.focusDistance = setting.focusDistance;
            if (
                setting.exposureMode !== undefined &&
                cameraCapabilities.exposureMode
            )
                adv.exposureMode = setting.exposureMode;
            if (
                setting.exposureTime !== undefined &&
                cameraCapabilities.exposureTime
            )
                adv.exposureTime = setting.exposureTime;
            if (
                setting.brightness !== undefined &&
                cameraCapabilities.brightness
            )
                adv.brightness = setting.brightness;
            if (setting.contrast !== undefined && cameraCapabilities.contrast)
                adv.contrast = setting.contrast;
            if (setting.sharpness !== undefined && cameraCapabilities.sharpness)
                adv.sharpness = setting.sharpness;
            if (
                setting.saturation !== undefined &&
                cameraCapabilities.saturation
            )
                adv.saturation = setting.saturation;
            if (setting.torch !== undefined && cameraCapabilities.torch)
                adv.torch = setting.torch;
    ...
    const startBulkScan = useCallback(() => {
        if (!videoRef.current || !videoReady || bulkScanIntervalRef.current)
            return;
                    ...
                    <div className="space-y-2">
                        <div className="relative overflow-hidden rounded-lg bg-black">
                            ...
                            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
                                ...
                                >
                                    <div className="absolute top-0 left-0 h-6 w-6 rounded-tl border-t-4 border-l-4 border-success" />
                                    <div className="absolute top-0 right-0 h-6 w-6 rounded-tr border-t-4 border-r-4 border-success" />
                                    <div className="absolute bottom-0 left-0 h-6 w-6 rounded-bl border-b-4 border-l-4 border-success" />
                                    <div className="absolute right-0 bottom-0 h-6 w-6 rounded-br border-r-4 border-b-4 border-success" />
                                </div>
                            </div>
                            {recognizing && (
                            ...
                            {bulkScanRunning && (
                                <div className="pointer-events-none absolute inset-0">
                                    ...
                                    <div className="absolute right-0 bottom-0 left-0 h-1.5 bg-black/30">
                                        <div
                        ...
                        {showCameraSettings && (
                            <div className="max-h-64 space-y-3 overflow-y-auto rounded-lg border bg-muted/30 p-3">
                                ...
                                {cameraCapabilities.focusMode &&
                                    cameraCapabilities.focusMode.length > 1 && (
                                        <div className="space-y-1">
                                            <span className="text-sm">
                                                Fokus-Modus
                                            </span>
                                            <div className="flex gap-1">
                                                {cameraCapabilities.focusMode.map(
                                                    (mode) => (
                                                        <Button
                                                            key={mode}
                                                            size="sm"
                                ...
                                {cameraCapabilities.sharpness && (
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            type="range"
                                            min={
                                                cameraCapabilities.sharpness.min
                                            }
                                            max={
                                                cameraCapabilities.sharpness.max
                                            }
                                            step={
                                                cameraCapabilities.sharpness
                                                    .step
                                ...
                                {cameraCapabilities.contrast && (
                                    <div className="space-y-1">
                                        ...
                                        <input
                                            type="range"
                                            min={
                                                cameraCapabilities.contrast.min
                                            }
                                            max={
                                                cameraCapabilities.contrast.max
                                            }
                                            step={
                                                cameraCapabilities.contrast.step
                                            }
                                ...
                                {cameraCapabilities.exposureMode &&
                                    cameraCapabilities.exposureMode.length >
                                        1 && (
                                        <div className="space-y-1">
                                            <span className="text-sm">
                                                Belichtung
                                            ...
                                            <div className="flex gap-1">
                                                {cameraCapabilities.exposureMode.map(
                                                    (mode) => (
                                                        <Button
                                                            key={mode}
                                                            size="sm"
                                ...
                                {!cameraCapabilities.zoom &&
                                    ...
                                    !cameraCapabilities.brightness &&
                                    (!cameraCapabilities.focusMode ||
                                        cameraCapabilities.focusMode.length <=
                                            1) && (
                                        <p className="text-xs text-muted-foreground">
                                            Diese Kamera unterstützt keine
